...
public class AdminFilterView extends Activity {
	...
	private AdminFilterAdapter  mClassScheduleAdapter;
	private ProgressDialog mPDialog;
	private AlertDialogManager alert = new AlertDialogManager();
	private String semID,secID,streamID,date_string;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		streamID = bundle.getString("StrmID");
		...
		ConnectionDetector conn = new ConnectionDetector(AdminFilterView.this);
		if(conn.isConnectingToInternet()){
			new TodayClassSchedule().execute();
		} else{
			alert.showAlertDialog(AdminFilterView.this,"Connection Error","Check your Internet Connection",false);
		}
	...
	class TodayClassSchedule extends AsyncTask<Void,Void,ArrayList<ClassScheduleModel>>{
		...
		protected void onPreExecute(){
			if(mPDialog==null){
				mPDialog = Util.createProgressDialog(AdminFilterView.this);
				mPDialog.show();
			} else mPDialog.show();
		}

		protected ArrayList<ClassScheduleModel> doInBackground(Void... params) {
			JSONObject receivedTimeTable  = HttpGetClient.sendHttpPost(Util.API+"timetable/"+date_string+"?stream_id="
					+streamID+"&semester_id="+semID+"&section_id="+secID+"&flag=mobile");
		...
		protected void onPostExecute(ArrayList<ClassScheduleModel> result){
			mPDialog.dismiss();
			if(result==null){
				//Toast.makeText(AdminFilterView.this, "No Class Schedule Found !", Toast.LENGTH_SHORT).show();
				...
				mClassScheduleAdapter = new AdminFilterAdapter(AdminFilterView.this,result);
