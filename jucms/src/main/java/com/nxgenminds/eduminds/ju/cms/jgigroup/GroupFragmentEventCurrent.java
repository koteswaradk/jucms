...
public class GroupFragmentEventCurrent extends Fragment{

	private SwipeRefreshLayout swipeLayout;
	private ListView eventscurrentlistview;
	private TextView noEvents;
	private ProgressDialog pDialog;
	
	private static String GroupEventsAPI = Util.API+"event?type=current&org_event=1";
	...
	public static final int group_current=555;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
		...
		eventscurrentlistview = (ListView) rootView.findViewById(R.id.eventscurrentlistview);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		eventscurrentlistview.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				if(eventscurrentlistview != null && eventscurrentlistview.getChildCount() > 0){
					boolean firstItemVisible = eventscurrentlistview.getFirstVisiblePosition() == 0;
					boolean topOfFirstItemVisible = eventscurrentlistview.getChildAt(0).getTop() == 0;
		...
		eventscurrentlistview.setOnItemClickListener(new OnItemClickListener() {

			@Override
			public void onItemClick(AdapterView<?> parent, View view,
					int position, long id) {
				// TODO Auto-generated method stub
				Object object = eventscurrentlistview.getItemAtPosition(position);
				...
				startActivityForResult(intent, group_current);
	...
	private class CurrentEventsAsyncClass extends AsyncTask<Void, Void, ArrayList<EventListGetModel>>{
		...
		protected void onPostExecute(ArrayList<EventListGetModel> result) {
			...
			if(result == null){
				...
				eventscurrentlistview.setAdapter(adapter);
				if(result.size() < 10){
					flag_loading = true;
				}else{
					flag_loading = false;
	...
	private class CurrentEventsLoadMoreAsyncClass extends AsyncTask<Void, Void, ArrayList<EventListGetModel>>{
		...
		protected void onPostExecute(ArrayList<EventListGetModel> result) {
			...
			if(result == null){
				...
				eventscurrentlistview.setSelection(pageCount);
			}
		}
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == group_current && resultCode == Activity.RESULT_OK && null != data)  
		{
