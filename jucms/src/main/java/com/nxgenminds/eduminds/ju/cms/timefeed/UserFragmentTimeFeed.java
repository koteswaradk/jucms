...
import com.nxgenminds.eduminds.ju.cms.adapters.PostsAdapter;
...
public class UserFragmentTimeFeed extends Fragment {

	private SwipeRefreshLayout swipeLayout;
	private ListView timelogListView;
	private TextView noFeed;
	private String UserTimeFeedURL = Util.API+ "post";
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		timelogListView = (ListView) rootView.findViewById(R.id.timeloglistView);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		adapter = new PostsAdapter(getActivity(), timeLogSearch);
		timeLogSearch.clear();
		pageCount = 0;
		...
		statusPost.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("Group", "User");
				startActivity(intent);
			}
		});

		noFeed.setOnClickListener(new OnClickListener() {
	...
	private class GetFeedRefreshAsyncClass extends AsyncTask<Void, Void, ArrayList<Posts>>{ 
		...
		protected ArrayList<Posts> doInBackground(Void... params) {
			JSONArray feedResponse = null;
			JSONObject jsonObjectRecived = HttpGetClient.sendHttpPost(UserTimeFeedURL+"?first_date="+ refresh_Date_String.replaceAll(" ", "%20"));
	...
	private class GetFeedLoadMoreAsyncClass extends AsyncTask<Void, Void, ArrayList<Posts>>{
		...
		protected ArrayList<Posts> doInBackground(Void... params) {
			JSONArray feedResponse = null;
			JSONObject jsonObjectRecived = HttpGetClient.sendHttpPost(UserTimeFeedURL+"?last_date="+ pagination_Date_String.replaceAll(" ", "%20"));
		...
		protected void onPostExecute(ArrayList<Posts> result) {
			...
			else if(HttpGetClient.statuscode == 200)
			{
				...
				else
				{
					...
					timelogListView.setSelection(pageCount);
				}
			}
		}
