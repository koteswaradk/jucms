...
public class EventListArchiveAdapter extends BaseAdapter {
	...
	public EventListArchiveAdapter(){
	}

	public EventListArchiveAdapter(Context context,ArrayList<EventListGetModel> listData){
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.attendenceText.setText("Attendees");
		...
		if(eventItem.getEvent_type_name().equalsIgnoreCase("feedback")){ // check for the feedback event
			...
			if(eventItem.getCreated_by().equalsIgnoreCase(Util.USER_ID)){
				...
				holder.noParticipationText.setVisibility(View.GONE);
				holder.eventEditIcon.setVisibility(View.GONE);
				holder.wentText.setVisibility(View.VISIBLE);

			} else {

				holder.eventEditIcon.setVisibility(View.GONE);

				if(eventItem.getLogin_user_participating().equalsIgnoreCase("1")&&!(eventItem.getCreated_by().equalsIgnoreCase(Util.USER_ID))){

					holder.noParticipationText.setVisibility(View.GONE);
					holder.wentText.setVisibility(View.VISIBLE);

				} 
				if(eventItem.getLogin_user_participating().equalsIgnoreCase("0")&&!(eventItem.getCreated_by().equalsIgnoreCase(Util.USER_ID))){ 


					holder.attendenceText.setText("Attendees");
					holder.noParticipationText.setVisibility(View.VISIBLE);
					holder.wentText.setVisibility(View.GONE);


				}
			}
		} // end of archive events for others type event.
		...
		//use profile click third party disabled 
