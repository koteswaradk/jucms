...
public class CompanyAdmin extends Fragment{
	...
	private class StudentsAsyncClass extends AsyncTask<Void, Void, ArrayList<ModelCompanyAdmin>>{
		...
		protected  ArrayList<ModelCompanyAdmin> doInBackground(Void... params) {
			...
			JSONObject jsonObjectRecived = HttpGetClient.sendHttpPost(Util.API+"worked_at?company_id="+Util.COMPANY_ID+"&currently_working=1");
	...
	private class StudentsLoadMoreAsyncClass extends AsyncTask<Void, Void, ArrayList<ModelCompanyAdmin>>{
		...
		protected ArrayList<ModelCompanyAdmin> doInBackground(Void... params) {
			JSONArray mStudentsResponse = null;
			JSONObject jsonObjectRecived = HttpGetClient.sendHttpPost(Util.API+"worked_at?company_id="+Util.COMPANY_ID+"&currently_working=1"+"&last_date=" + pagination_Date_String.replaceAll(" ", "%20"));
			if(jsonObjectRecived != null){
