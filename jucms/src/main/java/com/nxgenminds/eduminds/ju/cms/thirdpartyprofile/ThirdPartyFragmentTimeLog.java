...
public class ThirdPartyFragmentTimeLog extends Fragment{
	private SwipeRefreshLayout swipeLayout;
	private ListView timelogListView;
	private ProgressDialog pDialog;
	private TextView noFeed;
	private String GroupTimeFeedURL = Util.API+ "post?user_id=" + Util.THIRD_PARTY_ID;
	private ArrayList<Posts> timeLogSearch = new ArrayList<Posts>();
	private static int pageCount;
	private ThirdpartyTimelogAdapter adapter;
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		adapter = new ThirdpartyTimelogAdapter(getActivity(), timeLogSearch);
		timeLogSearch.clear();
		pageCount = 0;
			...
			else if(HttpGetClient.statuscode == 200)
			{
				try {
					if(result.getJSONArray("posts")== null)
					{
						...
						noFeed.setVisibility(View.VISIBLE);
						noFeed.setText("Check all the recent updates from all your connection right here! ");
